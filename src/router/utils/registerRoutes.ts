...
function convertRouteComponent(
  ...
  // 是否为一级菜单
  const isFirstLevel = depth === 1 && route.children?.length === 0;
  ...
  // 递归时增加深度
  if (children?.length) {
    converted.children = children.map((child: any) =>
 ...
 * 处理 iframe 类型路由
 */
function handleIframeRoute(converted: ConvertedRoute, route: AppRouteRecord, iframeRoutes: AppRouteRecord[]): void {
  converted.path = `/outside/iframe/${String(route.name)}`
  converted.component = () => import('@/views/careful-ui/outside/Iframe.vue')
 ...
 * 处理一级菜单路由
 */
function handleLayoutRoute(converted: any, route: any, component: string | undefined): void {
  converted.component = () => import('@/views/careful-ui/index/index.vue')

...
import {PostService} from "@/api/careful-ui/system/post";
...
const method = reactive({
  ...
  handleBatchDelete() {
    ...
    skyMsgBox("您确认需要进行批量删除么？")
      .then(async () => {
        try {
          await PostService.batchDelete(pageData.ids);
  ...
  async handleSubmit() {
    ...
    await formRef.value.validate(async (valid: any) => {
      if (valid) {
        if (pageData.form.id) {
          try {
            await PostService.update(pageData.form);
          ...
          try {
            await PostService.create(pageData.form);
  ...
  async handleDownload() {
    const res: any = await PostService.export(pageData.formFilters);
    // 创建下载链接
    ...
    link.href = url;
    link.setAttribute('download', `岗位信息导出_${new Date().toLocaleString()}.xlsx`);
  ...
  async handleListPage() {
    ...
    pageData.tableList = [];
    try {
      const res = await PostService.listPage({
        ...pageData.pagination,
            ...
            <el-tab-pane label="表单" name="表单">
              <el-form ref="formRef" :model="pageData.form" :rules="pageData.rules" label-width="80px">
                <el-form-item label="岗位名称" prop="name">
                  <el-input v-model="pageData.form.name" placeholder="岗位名称" :disabled="pageData.dialogType === 'edit'"/>
                </el-form-item>
                <el-form-item label="岗位编码" prop="code">
                  <el-input v-model="pageData.form.code" placeholder="岗位编码"/>
                </el-form-item>
